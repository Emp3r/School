...
(set-background *win1* :white)

(defun make-krizek ()
  (let ((k (make-instance 'polygon)))
    (set-color (set-filledp (set-closedp k t) t) :black)
    (set-items k (list (make-point 0 24)
                       (make-point 0 26)
                       (make-point 24 26)
                       (make-point 24 50)
                       (make-point 26 50)
                       ...
                       (make-point 24 0)
                       (make-point 24 24)))
    (rotate k (/ pi 4) (make-point 25 25))
    k))

(defun make-kolecko ()
  (let ((k (make-instance 'circle)))
    (set-x (set-y (center k) 25) 25)
    (set-radius k 19)
    (set-color (set-filledp k nil) :black)
    (set-thickness k 2)
    k))

(defun make-mrizka ()
  (let ((m (make-instance 'polygon)))
    (set-color (set-filledp (set-closedp m t) nil) :black)
    (set-thickness m 2)
    (set-items m (list (make-point 0 0)
                       (make-point 50 0)
                       (make-point 50 50)
                       (make-point 0 50)))
    m))
...
(defclass field (picture)
  ((state :initform nil)
   (position :initform (make-point 0 0))))

(defmethod get-position ((f field)) (slot-value f 'position))
(defmethod set-position ((f field) x y)
  (setf (slot-value f 'position) (make-point x y)))

(defmethod get-state ((f field)) (slot-value f 'state))
(defmethod set-state ((f field) n)
  (let ((xx (x (get-position f)))
        (yy (y (get-position f))))
    (cond ((= n 1) (setf (slot-value f 'items) (list (move (make-krizek) xx yy)
                                                     (move (make-mrizka) xx yy))))
          ((= n 2) (setf (slot-value f 'items) (list (move (make-kolecko) xx yy)
                                                     (move (make-mrizka) xx yy))))
          (t (setf (slot-value f 'items) (list (move (make-mrizka) xx yy)))))))

(defun make-field (xx yy)
  (let ((f (make-instance 'field)))
    (set-position f xx yy)
    (setf (slot-value f 'items) (list (move (make-mrizka) xx yy)))
    f))


;; TESTS ;;
(defvar field1 (make-field 0 0))
(set-state field1 0)
(defvar field2 (make-field 50 0))
(set-state field2 1)
(defvar field3 (make-field 0 50))
(set-state field3 2)
(defvar field4 (make-field 50 50))
(set-state field4 0)


(defvar all-fields (make-instance 'picture))
(set-items all-fields (list field1 field2 field3 field4))

(move all-fields 20 20)
;; (rotate all-fields (/ pi 4) (make-point 70 70))

(set-shape *win1* all-fields)
(redraw *win1*)
