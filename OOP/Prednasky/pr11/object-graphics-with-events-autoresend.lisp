...
(defmethod add-event ((obj mg-object) event)
  (if (listp event) (remove-event obj (car event)) ; repaired
...
(defmethod handlers ((obj mg-object))
  (slot-value obj 'handlers))

(defmethod set-handlers ((object mg-object) value) 
  (setf (slot-value object 'handlers) value)
  object)

(defmethod remove-handler ((obj mg-object) event)
  (setf (slot-value obj 'handlers)
	(remove-if (lambda (l)
		     (eql event (first l))) 
		   (slot-value obj 'handlers))))

(defmethod add-handler ((obj mg-object) event handler)
  (remove-handler obj event)
  (setf (slot-value obj 'handlers)
	(cons (list event handler)
	      (handlers obj)))
  obj)

(defmethod find-handler ((obj mg-object) event)
  (find-if (lambda (hand)
	     (eql event (car hand)))
	   (handlers obj)))

(defmethod event ((obj mg-object) ev-name sender &rest ev-args)
  (let ((h (find-handler obj ev-name)))
    (if h
        (apply (second h) obj sender ev-args)
      (apply #'send-event obj ev-name ev-args))))

(defmethod initialize-instance ((obj mg-object) &key)
  (call-next-method)
  (set-handlers obj `((ev-change ,(function ev-change)) (ev-changing ,(function ev-changing))))
  obj)

(defmethod ev-changing ((obj mg-object) sender message &rest message-args)
  (changing obj 'ev-changing sender message message-args))

(defmethod ev-change ((obj mg-object) sender message &rest message-args)
  (change obj 'ev-change sender message message-args))

; repaired

(defmethod send-event ((object mg-object) ev-name &rest event-args) 
  (when (delegate object)
    (let ((ev (second (find-event object ev-name)))) 
      (when ev 
        (apply #'event (delegate object) ev object event-args)))))
...
(defmethod initialize-instance ((w window) &key)
  ...
  (install-callbacks w)
  (set-handlers w `((ev-change ,(function ev-change)))) ; new
  w)
...
(defvar *circ*)
(setf *circ* (make-instance 'circle))
(set-x (center *circ*) 50)
(set-y (center *circ*) 50)
(set-radius *circ* 30)
(set-shape *win* *circ*)

(set-thickness *circ* 10)
(set-color *circ* :yellow)

(scale *circ* 2 (center *circ*))
(move *circ* 100 100)

(defvar *pol*)
(setf *pol* (make-instance 'polygon))
(set-color *pol* :green)
(set-filledp *pol* t)
(set-closedp *pol* t)
(set-color *pol* :lightblue)
(set-items *pol* (list (make-point 10 10)
                      (make-point 10 150)
                      (make-point 150 150)))
(set-shape *win* *pol*)
...
(set-items *pic* (list *circ* *pol*))
(set-shape *win* *pic*)

(set-delegate *pic* *del*)
(set-events *pic* '(ev-change ev-changing))
